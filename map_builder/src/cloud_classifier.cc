...
CloudClassifier::CloudClassifier(ros::NodeHandle* nh,
...
void CloudClassifier::loadFrameInfos(const std::string& infoDirectory) {
  ...
  while (std::getline(clusterFile, clusterLine) &&
    ...
    // Read pose data
    poseStream >> r1 >> r2 >> r3 >> r4 >> r5 >> r6 >> r7 >> r8 >> r9 >> x >>
        y >> z;
    ...
    if (clusterId == cluster_) {
      ...
      filePath << cloudDirectory_ << seq << "/3d_comp_os1_" << seq << "_"
               << frame << ".bin";

      // Convert to SE3 pose
      Eigen::Matrix3d rotationMatrix;
      rotationMatrix << r1, r2, r3, r4, r5, r6, r7, r8, r9;
...
std::vector<bool> CloudClassifier::queryPointCloud(size_t index) {
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr ground_cloud =
      extractPlanes(cloud, shiftedInliers, shiftedCoefficients,
                    groundThreshold_, groundAngle_, groundMaxIterations_);

  // 3 Set static points based on shiftedInliers
  for (size_t i = 0; i < cloud->size(); i++) {
    if (std::find(shiftedInliers->indices.begin(),
                  shiftedInliers->indices.end(),
                  i) != shiftedInliers->indices.end()) {
  ...
  // 5 Octree based KNN filtering
  std::vector<int> pointIdxNKNSearch;
  std::vector<float> pointNKNSquaredDistance;

  for (size_t i = 0; i < cloud->size(); ++i) {
    octree.nearestKSearch(cloud->points[i], K, pointIdxNKNSearch,
    ...
    if (!static_dynamic_flags[i] && count >= K) {
      static_dynamic_flags[i] = true;  // True if static, false if dynamic
    }
  }
...
void CloudClassifier::publishClassifiedPointCloud(
  ...
  for (size_t i = 0; i < cloud->size(); i++) {
    if (classification[i]) {
      cloud->points[i].r = 0;
      cloud->points[i].g = 0;
      cloud->points[i].b = 0;  // Black for static
    } else {
      cloud->points[i].r = 255;
      cloud->points[i].g = 0;
      cloud->points[i].b = 0;  // Red for dynamic
    }
  }
...
void CloudClassifier::loadPointCloud(const std::string& filepath,
  ...
  while (!file.eof()) {
    ...
    if (file.gcount() == sizeof(float)) cloud->push_back(point);
  }
